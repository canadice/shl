  ...
  mutate(
    newTeamID = 
      case_when(
        ...
        TeamId == 44 & Season >= 59 ~ 31,#Tampa Bay Barracuda new branding
...
history <- read.csv("csv/history_goalies.csv", sep = ",", dec = ".")
  ...
  mutate(
    newTeamID = 
      case_when(
        ...
        TeamId == 14 ~ 5, #Winnipeg Jets (WIN)
        TeamId == 14 & Season >= 59 ~ 32, #Winnipeg Aurora 
...
write.csv(history, file = "csv/history_goalies.csv", row.names = FALSE)
